    ...
    return (
        <div>
            ...
            <div className="grid md:grid-cols-2 gap-3 items-stretch">
                <Card className="h-full max-h-80 overflow-hidden">
                    <CardTitle>最近修改</CardTitle>
                    {recent.length === 0 ? (
                        <p className="text-sm text-ink-soft">暂无最近修改的文件</p>
                    ) : (
                        <div className="flex flex-col gap-1.5 max-h-72 overflow-hidden pr-1">
                            {recent.map((file) => (
                                <Link
                                    key={file.path}
                                    to={`/files/${
                ...
                <Link to="/storage" className="block h-full">
                    <Card className="h-full max-h-80 overflow-hidden hover:border-leaf/50 transition-colors">
