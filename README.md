...
| 文件管理 | 上传管理器 | 断点续传 |
|---|---|---|
| ✅ 目录树导航 + 上传/下载/重命名/移动/新建文件夹/分页 | ✅ 右下角常驻面板,多文件排队,暂停/继续/取消,可收起 | ✅ 大文件断网/刷新/重启后续传,下载支持 Range |

| 外部存储 | 在线压缩/解压 | 整盘导出/导入 |
|---|---|---|
| ✅ 挂载 S3/R2/MinIO,WebDAV 通用,不需桶配 CORS | ✅ 选中文件/文件夹压成 zip/tar.gz,在线解压 zip/tar.gz/tar.xz/tar | ✅ 打包网盘文件+配置库,换 VPS 导出导入 |

| 两种视图 | 文件夹图标 | 回收站 |
|---|---|---|
| ✅ 列表模式 + 缩略图模式(图片缩略图、视频封面) | ✅ 可给文件夹挑 emoji,目录树和列表同步显示 | ✅ 删除进垃圾桶,可还原或永久删除,30 天自动清理 |

| 全局搜索 | 在线预览 | Markdown 笔记 |
|---|---|---|
| ✅ 顶栏搜索全盘文件名(内存索引,60s 自动刷新) | ✅ Markdown/图片/视频/音频/文本/PDF + Word/Excel/PPT 全浏览器端渲染 | ✅ 左编辑右实时预览,Ctrl+S 保存 |

| 分享 | WebDAV | 离线下载 |
|---|---|---|
| ✅ 分享页(可设密码+过期)+直链,统一管理 | ✅ /dav/ 暴露整个网盘,与网页同端口 | ✅ aria2 承载,http/磁力/种子,网页化管理 |

| 动画登录页 | 黑夜模式 | 个性化 |
|---|---|---|
| ✅ 小家伙盯鼠标,输密码扭头回避 | ✅ 暖沙/暖炭双主题,切换过渡动画 | ✅ 自定义头像/用户名,WebDAV 同步生效 |
